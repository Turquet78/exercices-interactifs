...
module.exports = {
  ...
  'secondes.html': {
    ...
    liveCheck: null,
    pause: { dm: true, boxes: null },
    relance: { testId: 'pourcentage', kind: 'pct', fonction: 'startPercent' },
    rappels: null,
    specifique: null,
    lacunes: [
      "pas de fenêtre « Question à l'IA » (ni ouvrirQIA, ni qiaPoser, ni QIA_SUGG) : les 5 contrôles de la fenêtre détachée ne s'appliquent pas",
      "pas de <style id=\"ml-static-css\"> alors que mlDexp.tex() appelle convertLatexToMarkup : le premier exercice porté depuis la Première affichera « 10025 »",
      "window.dexpLiveCheck n'est jamais défini : la correction en direct du mode soutien ne se déclenche pas dans l'application réelle (liveCheckCurrent existe pourtant et gère pct/aug/dim/ens)",
      "captureBoxes ne lit que input[id] et select[id] : la pause perd les saisies math-field de l'élève",
